...
/**
 * Persistence seam for meters. Kept as a narrow interface over PrismaService
 * so the service layer can be tested without a database and the persistence
...
export interface MeterRepository {
  ...
  findByHouseholdId(householdId: string): Promise<Meter[]>;
  update(id: string, data: MeterUpdate): Promise<Meter | null>;
}

/** Fields of a meter that may be changed after creation. */
